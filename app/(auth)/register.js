...
function CampoInput({
  label, chave, value, onChange, placeholder,
  secure, keyboard, erros, setErros, setErroGeral,
}) {
  ...
  return (
    <View style={styles.campo}>
      ...
      <View style={styles.inputWrapper}>
        <TextInput
          style={[styles.input, ehSenha && styles.inputComIcone, erros[chave] && styles.inputErro]}
  ...
  async function handleCadastro() {
    ...
    try {
      await cadastrar({
        ...
        rm: "", id: "", course: "", semester: "",
        period: "", class: "", unidade: "", department: "",
      });
    ...
    >
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.papelContainer}>
          <TouchableOpacity
            style={[styles.papelBotao, papel === "student" && styles.papelBotaoAtivo]}
            onPress={() => setPapel("student")}
            activeOpacity={0.8}
          >
            <Ionicons name="school-outline" size={16} color={papel === "student" ? "#fff" : "#666"} />
            <Text style={[styles.papelTexto, papel === "student" && styles.papelTextoAtivo]}>
              Aluno
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.papelBotao, papel === "teacher" && styles.papelBotaoAtivo]}
            onPress={() => setPapel("teacher")}
            activeOpacity={0.8}
          >
            <Ionicons name="person-outline" size={16} color={papel === "teacher" ? "#fff" : "#666"} />
            <Text style={[styles.papelTexto, papel === "teacher" && styles.papelTextoAtivo]}>
              Professor
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <TouchableOpacity style={styles.botao} onPress={handleCadastro} disabled={loading} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
  ...
  subtitulo: { fontSize: 14, color: "#666", marginBottom: 20, textAlign: "center" },
  papelContainer: { flexDirection: "row", width: "100%", backgroundColor: "#fff", borderRadius: 12, padding: 4, marginBottom: 20, borderWidth: 1, borderColor: "#eee" },
  papelBotao: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, paddingVertical: 10, borderRadius: 10 },
