...
export default function Home() {
  ...
  const orderedDays = [...classroom.slice(startIndex), ...classroom.slice(0, startIndex)];
  ...
  const upcomingEvents = [...events]
    .filter(ev => parseDate(ev.date) >= new Date(new Date().setHours(0,0,0,0)))
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Confira suas aulas e próximos eventos</Text>
      </View>

      <View style={styles.swiperContainer}>
        <Swiper loop={false} showsPagination dotColor="#ddd" activeDotColor="#FF0C5C">
          {orderedDays.map((day, i) => (
            <View key={i} style={styles.slide}>
              <View style={styles.dayHeader}>
                <Text style={styles.day}>{day.name}</Text>
                {i === 0 && <View style={styles.todayBadge}><Text style={styles.todayText}>HOJE</Text></View>}
              </View>
      ...
      <View style={styles.sprintContainer}>
        <View style={styles.sprintCard}>
          ...
          {loading ? (
            <ActivityIndicator color="#FF0C5C" style={{ marginVertical: 12 }} />
            ...
            upcomingEvents.map((ev, i) => (
              <View key={i} style={styles.deliveryRow}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={styles.deliveryTitle}>{ev.title}</Text>
                  <Text style={styles.deliverySubject}>{ev.subject}</Text>
                </View>
                <Text style={styles.deliveryDate}>{ev.date}</Text>
              </View>
            ))
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  card: { width: "100%", height: 100, justifyContent: "center", backgroundColor: "#fff", paddingHorizontal: 15, borderRadius: 14, marginBottom: 12, elevation: 3 },
  ...
  classText: { color: "#444" },
  sprintContainer: { flexDirection: "row", justifyContent: "center", marginBottom: 24 },
  sprintCard: { width: "90%", backgroundColor: "#fff", padding: 15, borderRadius: 14, elevation: 3 },
  ...
  deliveryRow: { flexDirection: "row", alignItems: "center", marginTop: 8, paddingVertical: 6 },
