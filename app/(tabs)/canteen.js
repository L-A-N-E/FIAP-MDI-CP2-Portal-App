...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Dimensions, ActivityIndicator, Modal, TextInput } from "react-native";
...
const MENU_ITEMS = [
  ...
  { id: "pao-de-queijo", name: "Pão de queijo", price: 4.0,  defaultStock: 30 },
...
export default function Canteen() {
    ...
    async function load() {
      try {
        ...
        if (sJson) {
          ...
          const inicial = {};
          MENU_ITEMS.forEach(i => { inicial[i.id] = i.defaultStock; });
          await AsyncStorage.setItem(STOCK_KEY, JSON.stringify(inicial));
          setStock(inicial);
        }
  ...
  async function concluirPedido() {
    ...
    try {
      ...
      const numero = (orders.length > 0 ? Math.max(...orders.map(o => o.numero)) : 10) + 1;
  ...
  async function salvarEstoque() {
    const novoStock = { ...stock, [editItem.id]: Math.max(0, parseInt(editValor) || 0) };
    await AsyncStorage.setItem(STOCK_KEY, JSON.stringify(novoStock));
    setStock(novoStock);
    setEditModal(false);
  }
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.tabs, { width: maxW }]}>
          {["pedidos", "fila", "historico"].map(p => (
            <TouchableOpacity
              key={p}
              style={[styles.tab, pagina === p && styles.tabActive]}
              onPress={() => setPagina(p)}
              activeOpacity={0.85}
            >
              <Text style={[styles.tabText, pagina === p && styles.tabTextActive]}>
                {p === "pedidos" ? "Pedido" : p === "fila" ? "Fila" : "Histórico"}
              </Text>
            </TouchableOpacity>
          ))}
        </View>
          ...
          <View style={{ width: maxW }}>
                ...
                <View key={item.id} style={styles.card}>
                  <View style={styles.rowBetween}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={[styles.stockText, !estoqueOk && styles.stockVazio]}>
                        {estoqueOk ? `Estoque: ${stock[item.id]}` : "Sem estoque"}
                      </Text>
                    </View>
                    ...
                    <View style={styles.qtyControls}>
                      {isTeacher ? (
                        ...
                        <TouchableOpacity style={styles.editBtn} onPress={() => abrirEdicaoEstoque(item)}>
                          <Text style={styles.editBtnText}>Editar estoque</Text>
                        </TouchableOpacity>
                      ) : (
                        <>
                          <TouchableOpacity
                          ...
                          <TouchableOpacity
                            style={[styles.qtyButton, !estoqueOk && styles.qtyButtonDisabled]}
                            onPress={() => alterarQtd(item.id, +1)}
                            disabled={!estoqueOk}
                          >
                            <Text style={styles.qtyButtonText}>+</Text>
            ...
            {!isTeacher && (
              <View style={styles.summaryCard}>
                ...
                <TouchableOpacity
                  style={[styles.primaryButton, (cartCount === 0 || submitting) && styles.primaryButtonDisabled]}
          ...
          <View style={{ width: maxW }}>
            <View style={styles.card}>
              ...
              <View style={styles.queueGrid}>
                ...
                <View style={styles.queueBox}>
                  <Text style={styles.queueLabel}>Total na fila</Text>
                  <Text style={styles.queueNumber}>{orders.length}</Text>
                </View>
              </View>
              {!!mensagem && <Text style={styles.feedback}>{mensagem}</Text>}
            </View>
            <TouchableOpacity style={styles.secondaryButton} onPress={() => { setPagina("pedidos"); setMeuNumero(null); setMensagem(""); }}>
            ...
            {meusOrders.length === 0 ? (
              ...
              meusOrders.map((o, i) => (
                <View key={i} style={styles.card}>
                  <View style={styles.rowBetween}>
                    <Text style={styles.itemName}>Pedido #{o.numero}</Text>
                    <Text style={styles.itemPrice}>{formatBRL(o.total)}</Text>
                  </View>
                  <Text style={{ color: "#999", fontSize: 12, marginTop: 2 }}>{o.data}</Text>
                  {o.itens.map((it, j) => (
                    <Text key={j} style={{ color: "#444", fontSize: 13, marginTop: 4 }}>
                      • {it.name} x{it.qty}
                    </Text>
                  ))}
                </View>
      ...
      <Modal visible={editModal} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modal}>
            <Text style={styles.modalTitle}>Editar estoque — {editItem?.name}</Text>
            <TextInput
            ...
            <View style={styles.modalBtns}>
              ...
              <TouchableOpacity style={styles.modalBtnSave} onPress={salvarEstoque}>
...
const styles = StyleSheet.create({
  ...
  scrollContent: { paddingBottom: 26, alignItems: "center", paddingHorizontal: 16 },
  ...
  tabs: { flexDirection: "row", backgroundColor: "#fff", borderRadius: 14, padding: 6, elevation: 2, marginBottom: 14 },
  tab: { flex: 1, paddingVertical: 10, borderRadius: 10, alignItems: "center" },
  tabActive: { backgroundColor: "#FF0C5C" },
  tabText: { fontWeight: "bold", color: "#666" },
  tabTextActive: { color: "#fff" },
  sectionHeader: { flexDirection: "row", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 10, paddingHorizontal: 2 },
  sectionTitle: { fontSize: 18, fontWeight: "bold" },
  sectionHint: { fontSize: 12, color: "#666", fontWeight: "bold" },
  card: { backgroundColor: "#fff", borderRadius: 14, padding: 16, elevation: 3, marginBottom: 12, width: "100%" },
  emptyCard: { backgroundColor: "#fff", borderRadius: 14, padding: 18, elevation: 2, marginBottom: 12, alignItems: "center", width: "100%" },
  ...
  editBtn: { backgroundColor: "#FF0C5C", paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8 },
  editBtnText: { color: "#fff", fontWeight: "bold", fontSize: 12 },
  qtyControls: { flexDirection: "row", alignItems: "center", gap: 8 },
  qtyButton: { width: 36, height: 36, borderRadius: 10, backgroundColor: "#FF0C5C", alignItems: "center", justifyContent: "center" },
  qtyButtonDisabled: { backgroundColor: "#ff7aa5" },
  qtyButtonText: { color: "#fff", fontSize: 18, fontWeight: "bold" },
  qtyBadge: { minWidth: 36, height: 36, borderRadius: 10, backgroundColor: "#f5f5f5", alignItems: "center", justifyContent: "center", paddingHorizontal: 8 },
  qtyText: { fontWeight: "bold", color: "#111" },
  summaryCard: { marginTop: 4, backgroundColor: "#fff", borderRadius: 14, padding: 16, elevation: 3, width: "100%" },
  summaryLabel: { color: "#666", fontWeight: "bold" },
  summaryValue: { fontSize: 16, fontWeight: "bold", color: "#111" },
  feedback: { marginTop: 10, textAlign: "center", color: "#666", fontWeight: "bold" },
  primaryButton: { marginTop: 12, backgroundColor: "#FF0C5C", paddingVertical: 14, borderRadius: 12, alignItems: "center" },
  primaryButtonDisabled: { backgroundColor: "#ff7aa5" },
  primaryButtonText: { color: "#fff", fontWeight: "bold", fontSize: 16 },
  queueTitle: { fontSize: 18, fontWeight: "bold", textAlign: "center", marginBottom: 12 },
  queueGrid: { flexDirection: "row", gap: 12 },
  queueBox: { flex: 1, backgroundColor: "#f5f5f5", borderRadius: 14, paddingVertical: 14, alignItems: "center" },
  queueLabel: { color: "#666", fontWeight: "bold", marginBottom: 8 },
  queueNumber: { fontSize: 26, fontWeight: "bold", color: "#FF0C5C" },
  secondaryButton: { marginTop: 12, backgroundColor: "#111", paddingVertical: 14, borderRadius: 12, alignItems: "center", width: "100%" },
  ...
  modalInput: { borderWidth: 1, borderColor: "#ddd", borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12, fontSize: 15, marginBottom: 16 },
